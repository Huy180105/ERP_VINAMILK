...
class SalesStock
{
    ...
    public function reservations(array $lots, ?string $exclude = null, bool $lock = false)
    {
        $query = DB::table('ChiTietPhieuXuatSP as ct')->join('PhieuXuatSP as px', 'px.maPhieuXuatSP', '=', 'ct.maPhieuXuatSP')
            ->whereIn('ct.maTonKho', $lots)->whereNotIn('px.trangThai', ['Đã xuất kho', 'Hoàn thành', 'Đã hủy'])
    ...
    public function complete(string $id)
    {
        return DB::transaction(function () use ($id) {
            ...
            if (in_array($dispatch->trangThai, ['Đã xuất kho', 'Hoàn thành'])) {
                if ($order) {
            ...
            if ($order) {
                ...
                $exported = DB::table('ChiTietPhieuXuatSP as ct')->join('PhieuXuatSP as px', 'px.maPhieuXuatSP', '=', 'ct.maPhieuXuatSP')
                    ->join('TonKho as tk', 'tk.maTonKho', '=', 'ct.maTonKho')->where('px.maDonHang', $order->maDonHang)->whereIn('px.trangThai', ['Đã xuất kho', 'Hoàn thành'])
