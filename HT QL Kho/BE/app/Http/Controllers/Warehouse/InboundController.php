...
class InboundController extends Controller
{
    ...
    public function getPendingProductionHandovers()
    {
        $handovers = PhieuYeuCauXuatSP::with(['chiTiets.sanPham', 'phieuNghiemThu', 'nhanVien'])
            ->whereIn('trangThai', ['Chưa xác nhận', 'Chưa xử lý'])
    ...
    public function createProductReceipt(Request $request)
    {
        ...
        try {
            ...
            // Cập nhật trạng thái PhieuYeuCauXuatSP → Chờ xác nhận (kho đã tạo phiếu nhập)
    ...
    public function approveProductReceipt($id)
    {
        ...
        try {
            ...
            // Đồng bộ: PhieuYeuCauXuatSP → Đã duyệt
            if ($receipt->maPhieuYCXSP) {
                PhieuYeuCauXuatSP::where('maPhieuYCXSP', $receipt->maPhieuYCXSP)
            ...
            DB::rollBack();
            return response()->json(['success' => false, 'message' => 'Lỗi duyệt phiếu: ' . $e->getMessage()], 500);
        }
    ...
    public function rejectProductReceipt($id)
    {
        ...
        try {
            ...
            // Đồng bộ: PhieuYeuCauXuatSP → Không duyệt
            if ($receipt->maPhieuYCXSP) {
                PhieuYeuCauXuatSP::where('maPhieuYCXSP', $receipt->maPhieuYCXSP)
                    ->update(['trangThai' => 'Không duyệt']);
            }

            DB::commit();
        } catch (\Exception $e) {
            DB::rollBack();
            return response()->json(['success' => false, 'message' => 'Lỗi từ chối phiếu: ' . $e->getMessage()], 500);
        }
    ...
    public function confirmGoodsReceived($id)
    {
        ...
        if ($receipt->trangThai !== 'Đã duyệt') {
            return response()->json([
                'success' => false,
                'message' => "Phiếu nhập phải ở trạng thái 'Đã duyệt' mới có thể xác nhận lấy hàng. Trạng thái hiện tại: {$receipt->trangThai}",
            ], 400);
        }
        ...
        try {
            ...
            // Đồng bộ: PhieuYeuCauXuatSP → Thành công (kho xác nhận lấy hàng xong)
            if ($receipt->maPhieuYCXSP) {
                PhieuYeuCauXuatSP::where('maPhieuYCXSP', $receipt->maPhieuYCXSP)
            ...
            return response()->json([
                'success' => true,
                'message' => 'Xác nhận lấy hàng thành công! Trạng thái phiếu và yêu cầu xuất kho đã chuyển thành Thành công.',
                'data'    => $receipt,
            ]);
        } catch (\Exception $e) {
            DB::rollBack();
            return response()->json(['success' => false, 'message' => 'Lỗi xác nhận lấy hàng: ' . $e->getMessage()], 500);
        }
    }
