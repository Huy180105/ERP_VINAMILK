...
class OutboundController extends Controller
{
    ...
    public function approveRawMaterialDispatch($id)
    {
        ...
        if ($dispatch->trangThai !== 'Chờ duyệt') {
            return response()->json([
                'success' => false,
                'message' => "Phiếu xuất đang ở trạng thái '{$dispatch->trangThai}', không thể duyệt.",
            ], 400);
        }
        ...
        try {
            ...
            // Đồng bộ: PhieuYeuCauNVL → Đã duyệt
    ...
    public function rejectRawMaterialDispatch(Request $request, $id)
    {
        ...
        try {
            ...
            // Đồng bộ: PhieuYeuCauNVL → Không duyệt
            if ($dispatch->maPhieuYeuCauNVL) {
                PhieuYeuCauNVL::where('maPhieuYCNVL', $dispatch->maPhieuYeuCauNVL)
    ...
    public function completeRawMaterialDispatch($id)
    {
        ...
        if ($dispatch->trangThai === 'Đã xuất kho') {
        ...
        try {
            ...
            // Đồng bộ: PhieuYeuCauNVL → Đã xuất kho
            if ($dispatch->maPhieuYeuCauNVL) {
                PhieuYeuCauNVL::where('maPhieuYCNVL', $dispatch->maPhieuYeuCauNVL)
            ...
            return response()->json([
                'success' => true,
                'message' => 'Xác nhận xuất kho NVL thành công! Tồn kho đã được trừ. Trạng thái phiếu yêu cầu NVL đã chuyển sang Đã xuất kho.',
                'data'    => $dispatch,
            ]);
        } catch (\Exception $e) {
            DB::rollBack();
