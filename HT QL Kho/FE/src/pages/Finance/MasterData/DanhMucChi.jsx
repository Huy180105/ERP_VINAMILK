...
export default function DanhMucChi() {
  ...
  const openCreateModal = () => {
    setEditingItem(null);
    setFormData({
      maDanhMucChi: `DMC${String(Date.now()).slice(-4)}`,
      tenDanhMucChi: '',
      moTa: '',
      trangThai: 1,
    });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingItem) {
        await FinanceMasterDataAPI.updateExpCategory(editingItem.maDanhMucChi, formData);
      } else {
        await FinanceMasterDataAPI.createExpCategory(formData);
      }
      setModalOpen(false);
      fetchData();
    } catch (err) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white p-5 rounded-lg border border-slate-200 shadow-sm">
        <div>
          <h1 className="text-xl font-bold text-slate-800 flex items-center space-x-2">
            <ListTree className="w-6 h-6 text-rose-600" />
      ...
      <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-sm flex items-center justify-between">
        <div className="relative w-80">
          <input
            type="text"
            placeholder="Tìm theo mã hoặc tên khoản chi..."
            value={keyword}
            onChange={(e) => setKeyword(e.target.value)}
            className="w-full bg-slate-50 border border-slate-200 text-xs text-slate-800 rounded-md pl-9 pr-4 py-2 focus:outline-none focus:ring-2 focus:ring-rose-300"
          />
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-50 text-slate-500 font-semibold uppercase text-[10px] border-b border-slate-200">
              <tr>
                <th className="p-3.5">Mã Khoản Chi</th>
                <th className="p-3.5">Tên Khoản Mục Chi</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                <tr><td colSpan="5" className="p-6 text-center text-slate-400">Không tìm thấy khoản mục chi nào</td></tr>
              ) : (
                categories.map((c) => (
                  <tr key={c.maDanhMucChi} className="hover:bg-slate-50 transition">
                    <td className="p-3.5 font-mono font-bold text-rose-900">{c.maDanhMucChi}</td>
        ...
        <div className="fixed inset-0 bg-slate-900/50 -xs flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-sm max-w-md w-full p-6 space-y-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Mã Khoản Chi:</label>
                <input
                  type="text"
                  value={formData.maDanhMucChi}
              ...
              <div>
                ...
                <textarea
                  rows="3"
                  placeholder="Mô tả mục đích khoản chi..."
                  value={formData.moTa}
              ...
              <div className="flex justify-end space-x-2 pt-2 border-t">
                ...
                <button
                  type="submit"
                  className="px-4 py-2 rounded-md text-xs font-bold bg-rose-600 hover:bg-rose-700 text-white shadow-sm"
                >
