  ...
  const handlePaymentSubmit = async (e) => {
    ...
    try {
      const res = await SalesAPI.recordPayment({
        maThanhToan: `TT${crypto.randomUUID().replaceAll('-', '').slice(0, 18).toUpperCase()}`,
        maCongNo: selectedInvoice.maCongNo,
        soTien: paymentFormData.soTienThanhToan,
        phuongThuc: paymentFormData.phuongThuc,
        thamChieu: paymentFormData.phuongThuc === 'Chuyển khoản' ? `HD-${selectedInvoice.maHoaDon}` : null,
      });
      if (res.data?.warning) {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-slate-200/80 shadow-xs">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            Hóa đơn được tạo tự động ngay khi lập đơn hàng; theo dõi thanh toán và công nợ tại đây.
          </p>
        </div>
      </div>
                ...
                filteredInvoices.map((inv) => (
                  <tr key={inv.maHoaDon} className="hover:bg-blue-50/40 transition-colors">
                    ...
                    <td className="px-5 py-3.5 font-mono text-[11px] text-slate-600">
                      <div>{inv.maGiaoHang || 'Chờ Kho xác nhận'}</div>
