...
export default function SalesOrders() {
  ...
  const openEditModal = (order) => {
    if (!['Chờ kho xác nhận', 'Chờ xác nhận'].includes(order.trangThai)) {
      notify('error', 'Chỉ đơn hàng đang chờ Kho xác nhận mới được phép chỉnh sửa.');
      return;
    }
  ...
  const handleDeleteOrder = async (order) => {
    if (!window.confirm(`Hủy đơn hàng ${order.maDonHang}? Chỉ có thể hủy trước khi Kho xác nhận.`)) return;

    try {
      await SalesAPI.updateOrderStatus(order.maDonHang, 'Đã hủy');
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-slate-200/80 shadow-xs">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            Lưu đơn sẽ kiểm tra tồn kho, tạo hóa đơn tự động và chuyển sang Kho (NB) xác nhận.
          </p>
        </div>
                  ...
                  <tr key={o.maDonHang} className="hover:bg-blue-50/40 transition-colors">
                    ...
                    <td className="px-5 py-3.5 text-center">
                      <div className="flex items-center justify-center gap-1.5">
                        {['Chờ kho xác nhận', 'Chờ xác nhận'].includes(o.trangThai) && (
                          <>
                            <button
